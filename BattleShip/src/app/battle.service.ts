...
import { BattleArea } from './battle-area/model/battle-area.model';
import { Player } from './battle-area/model/player.model';
    ...
    if(shipList){
      for(let ship of shipList){
        for(let cell of ship.cellList){
          if(cell.cellValue === missile && cell.strength > 0){
            cell.strength--;
            ship.totalShipStrength--;
            console.log(`missile : ${missile} cell strength : ${cell.strength}, ship strength : {$ship.totalShipStrength}`)
            return true;
          }
        }
      }
